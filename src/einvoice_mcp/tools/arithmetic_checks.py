...
def check_arithmetic(checks: list[FieldCheck], root: Element) -> None:
    ...
    allowance_total = _safe_amount(root.find(f"{ms_prefix}/ram:AllowanceTotalAmount", CII_NS))
    charge_total = _safe_amount(root.find(f"{ms_prefix}/ram:ChargeTotalAmount", CII_NS))
    tax_basis = _safe_amount(root.find(f"{ms_prefix}/ram:TaxBasisTotalAmount", CII_NS))
    tax_total = _safe_amount(root.find(f"{ms_prefix}/ram:TaxTotalAmount", CII_NS))
    grand_total = _safe_amount(root.find(f"{ms_prefix}/ram:GrandTotalAmount", CII_NS))
    prepaid = _safe_amount(root.find(f"{ms_prefix}/ram:TotalPrepaidAmount", CII_NS))
    due_payable = _safe_amount(root.find(f"{ms_prefix}/ram:DuePayableAmount", CII_NS))
    ...
    if line_total is not None and tax_basis is not None:
        expected_basis = (
            line_total - (allowance_total or Decimal("0")) + (charge_total or Decimal("0"))
        )
