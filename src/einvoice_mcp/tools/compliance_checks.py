...
def check_fields(xml_content: str, *, xrechnung: bool = True) -> list[FieldCheck]:
    ...
    tax_regs = root.findall(".//ram:SellerTradeParty/ram:SpecifiedTaxRegistration/ram:ID", CII_NS)
...
def _check_sepa_iban(checks: list[FieldCheck], root: Element, pm_code: str) -> None:
    ...
    iban_value = (
        iban_el.text.strip() if iban_present and iban_el is not None and iban_el.text else ""
    )
    checks.append(
        FieldCheck(
            field="BT-84",
...
def _check_preceding_invoice(checks: list[FieldCheck], root: Element) -> None:
    ...
    type_code_val = (
        type_code_el.text.strip() if type_code_el is not None and type_code_el.text else ""
    )
    ...
    bt25_present = inv_ref_el is not None and bool(inv_ref_el.text and inv_ref_el.text.strip())
    bt25_value = (
...
def _check_tax_rate_zero(
    ...
    for tax_el in root.findall(".//ram:ApplicableTradeTax", CII_NS):
        ...
        if (
            cat_el is not None
            and cat_el.text
            and cat_el.text.strip() == category
...
def _check_tax_categories(
    ...
    if has_k:
        ...
        has_buyer_vat = any(el.get("schemeID") == "VA" and el.text for el in buyer_tax_regs_k)
...
def _check_non_de_seller_tax_rep(checks: list[FieldCheck], root: Element) -> None:
    ...
    if not has_rep:
        checks.append(
            FieldCheck(
                field="BR-DE-3",
                name="Steuervertreter bei nicht-DE Verkäufer",
                present=False,
                value=seller_country,
                required=True,
            )
        )
...
def _check_seller_contact(checks: list[FieldCheck], root: Element) -> None:
    ...
    phone_el = root.find(
        f"{contact_base}/ram:TelephoneUniversalCommunication/ram:CompleteNumber",
        CII_NS,
    )
    email_el = root.find(
        f"{contact_base}/ram:EmailURIUniversalCommunication/ram:URIID",
        CII_NS,
    )
    ...
    if not (phone_el is not None and phone_el.text and phone_el.text.strip()):
        checks.append(
            FieldCheck(
                field="BR-DE-6",
                name="Telefon Ansprechpartner (BT-42)",
                present=False,
                value="",
                required=True,
            )
    ...
    if not (email_el is not None and email_el.text and email_el.text.strip()):
        checks.append(
            FieldCheck(
                field="BR-DE-7",
                name="E-Mail Ansprechpartner (BT-43)",
                present=False,
                value="",
                required=True,
            )
...
def _check_payment_means_rules(checks: list[FieldCheck], root: Element) -> None:
    ...
    if not pm_elements:
        checks.append(
            FieldCheck(
                field="BR-DE-18",
                name="Zahlungsart (PaymentMeansCode)",
                present=False,
                value="",
                required=True,
            )
        ...
        if pm_el.text:
            ...
            if code and code not in _VALID_PM_CODES:
                checks.append(
                    FieldCheck(
                        field="BR-DE-19",
                        name="Zahlungsart-Code ungültig",
                        present=False,
                        value=code,
                        required=True,
                    )
    ...
    if seller_ea_el is not None:
        ...
        if scheme and scheme not in _VALID_EAS:
            checks.append(
                FieldCheck(
                    field="BR-DE-16",
                    name="Verkäufer-EAS-Code ungültig",
                    present=False,
                    value=scheme,
                    required=True,
                )
...
def _check_leitweg_format(
    ...
    if buyer_ref_el is not None and buyer_ref_el.text:
        ...
        if leitweg and not re.match(r"^[0-9]{2,12}-[0-9A-Za-z]{1,30}-[0-9A-Za-z]{2,5}$", leitweg):
            checks.append(
                FieldCheck(
                    field="LW-FMT",
...
def _check_date_formats(checks: list[FieldCheck], root: Element) -> None:
    ...
    date_checks = [
        ...
        (
            "BR-DE-10",
            "Leistungszeitraum Beginn Datumsformat",
            ".//ram:ApplicableHeaderTradeSettlement"
        ...
        (
            "BR-DE-11",
            "Leistungszeitraum Ende Datumsformat",
            ".//ram:ApplicableHeaderTradeSettlement"
            "/ram:BillingSpecifiedPeriod"
